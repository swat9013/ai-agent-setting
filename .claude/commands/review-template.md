...
1. `template/` ディレクトリが存在する
2. `docs/design.md` が存在し読み込み可能
3. `CLAUDE.md` が存在し読み込み可能
...
1. すべてのレビュー観点（設計原則、一貫性、汎用性、完全性）がチェックされている
2. 各指摘に具体的な改善案が含まれている
3. 問題点が優先度別（Critical/Warning/Info）に整理されている
