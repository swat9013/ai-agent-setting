...
1. **コンテキストの成長** - 会話から得た知見を蓄積し、プロジェクトとともに進化させる
2. **確実な実行** - 育てた知見がAIに確実に参照・実行される仕組みを提供する
...
context.md はプロンプトと同様に磨くべき。定期的に実験し、何が効くか確認する。

```
1. 観察 ─→ AIの挙動で期待と異なる点を記録
     │
2. 仮説 ─→ context.mdのどの部分が原因か推測
     │
3. 変更 ─→ 小さな修正を加える（1-2箇所）
     │
4. 検証 ─→ 同様のタスクで効果を確認
     │
5. 記録 ─→ 効果があった変更をADRまたはコメントで記録
     │
     └──→ 1. へ戻る
```

**ポイント:**
- 一度に多くの変更を加えない（効果が測定できなくなる）
- 効果がなかった変更も記録する（同じ失敗を繰り返さない）
- 定期的に振り返りの時間を設ける

### 10.4 視覚的参照の活用

UIコンポーネント実装時は、デザインモックやスクリーンショットをAIに渡すと精度が向上する。

```markdown
<!-- 指示例 -->
添付のデザインモックを参考に、ユーザープロフィールカードを実装してください。
「美しく見える」ことを意識した実装をお願いします。
```
